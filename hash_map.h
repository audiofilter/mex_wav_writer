...
// Copy from SPUC except this one has a destructor that delete the components
// hence the must be pointers
#include <stdexcept>
#include <string>
...
template<typename T> class hash_map  {
 ...
 public:
  ...
  typename std::unordered_map<std::string, T>::const_iterator _iter; // Iterator
	...
	//! Delete the elements
	~hash_map() {
		for (_iter = table.begin(); _iter != table.end(); _iter++) {
			std::string name = _iter->first;
			T wav = _iter->second;
			delete wav;
		}
